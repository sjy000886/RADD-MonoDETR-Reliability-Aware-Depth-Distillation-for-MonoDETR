...
class KITTI_Dataset(data.Dataset):
    def __init__(self, split, cfg):
        ...
        # Optional dense depth generated by CompletionFormer (or another
        # external teacher). It regularizes training features and is loaded
        # only for the configured splits.
        teacher_cfg = cfg.get('teacher_depth', {})
        teacher_splits = teacher_cfg.get('splits', ['train', 'trainval'])
        self.teacher_depth_enabled = (
            bool(teacher_cfg.get('enabled', False)) and split in teacher_splits)
        self.teacher_depth_root = teacher_cfg.get('root_dir')
        self.teacher_depth_template = teacher_cfg.get(
            'filename_template', '{img_id:06d}.png')
        self.teacher_depth_value_scale = float(teacher_cfg.get('value_scale', 256.0))
        self.teacher_depth_npz_key = teacher_cfg.get('npz_key', 'depth')
        self.teacher_depth_missing = teacher_cfg.get('missing_policy', 'error').lower()
        self.teacher_confidence_root = teacher_cfg.get('confidence_root_dir')
        self.teacher_confidence_template = teacher_cfg.get(
            'confidence_filename_template', '{img_id:06d}.npy')
        self.teacher_confidence_threshold = float(
            teacher_cfg.get('confidence_threshold', 0.0))
        self.teacher_depth_min = float(teacher_cfg.get('min_depth', 1e-3))
        self.teacher_depth_max = float(teacher_cfg.get('max_depth', 60.0))
        self.teacher_depth_interpolation = teacher_cfg.get(
            'augmentation_interpolation', 'nearest').lower()

        if self.teacher_depth_enabled:
            if not self.teacher_depth_root:
                raise ValueError(
                    'teacher_depth.root_dir is required when teacher supervision is enabled')
            if not self.teacher_confidence_root:
                raise ValueError(
                    'teacher_depth.confidence_root_dir is required when teacher '
                    'supervision is enabled')
            if self.teacher_depth_value_scale <= 0:
                raise ValueError('teacher_depth.value_scale must be positive')
            if self.teacher_depth_missing not in ['error', 'zeros']:
                raise ValueError(
                    "teacher_depth.missing_policy must be 'error' or 'zeros'")
            if self.teacher_depth_interpolation not in ['nearest', 'masked_bilinear']:
                raise ValueError(
                    "teacher_depth.augmentation_interpolation must be 'nearest' "
                    "or 'masked_bilinear'")
    ...
    def _load_teacher_array(self, path, npz_key):
        ...
        if extension == '.npz':
            with np.load(path) as archive:
                if npz_key not in archive:
                    raise KeyError(
                        "Key '{}' not found in teacher file {}".format(
                            npz_key, path))
                return archive[npz_key]
        return np.asarray(Image.open(path))

    def get_teacher_depth(self, idx, image_size):
        relative_path = self.teacher_depth_template.format(
            img_id=idx, index=idx, split=self.split)
        depth_file = os.path.join(self.teacher_depth_root, relative_path)
        if not os.path.exists(depth_file):
            if self.teacher_depth_missing == 'zeros':
                shape = (image_size[1], image_size[0])
                return (np.zeros(shape, dtype=np.float32),
                        np.zeros(shape, dtype=np.float32))
            raise FileNotFoundError(
                'Teacher depth map not found for KITTI image {}: {}'.format(
                    idx, depth_file))

        confidence_relative_path = self.teacher_confidence_template.format(
            img_id=idx, index=idx, split=self.split)
        confidence_file = os.path.join(
            self.teacher_confidence_root, confidence_relative_path)
        if not os.path.exists(confidence_file):
            if self.teacher_depth_missing == 'zeros':
                ...
                return (np.zeros(shape, dtype=np.float32),
                        np.zeros(shape, dtype=np.float32))
            raise FileNotFoundError(
                'Teacher confidence map not found for KITTI image {}: {}'.format(
                    idx, confidence_file))

        depth = self._load_teacher_array(depth_file, self.teacher_depth_npz_key)
        confidence = self._load_teacher_array(confidence_file, 'confidence')
        ...
        if depth.ndim != 2 or depth.shape != expected_shape:
            raise ValueError(
                'Teacher depth {} has shape {}, expected {}'.format(
                    depth_file, depth.shape, expected_shape))
        if confidence.ndim != 2 or confidence.shape != expected_shape:
            raise ValueError(
                'Teacher confidence {} has shape {}, expected {}'.format(
                    confidence_file, confidence.shape, expected_shape))
        depth = depth.astype(np.float32) / self.teacher_depth_value_scale
        return depth, confidence.astype(np.float32)

    def transform_teacher_depth(self, depth, confidence, trans_inv,
                                random_flip_flag, crop_scale):
        depth_valid = (
            np.isfinite(depth)
            & (depth >= self.teacher_depth_min)
            & (depth <= self.teacher_depth_max)
        )
        confidence = np.where(np.isfinite(confidence), confidence, 0).astype(
            np.float32)
        valid = depth_valid & (confidence > self.teacher_confidence_threshold)
        ...
        if random_flip_flag:
            ...
            confidence = np.ascontiguousarray(np.fliplr(confidence))
            valid = np.ascontiguousarray(np.fliplr(valid))

        transform_data = tuple(trans_inv.reshape(-1).tolist())
        output_size = tuple(self.resolution.tolist())
        if self.teacher_depth_interpolation == 'nearest':
            depth = np.array(Image.fromarray(depth).transform(
                output_size, method=Image.AFFINE, data=transform_data,
                resample=Image.NEAREST), dtype=np.float32, copy=True)
            valid = np.array(Image.fromarray(valid).transform(
                output_size, method=Image.AFFINE, data=transform_data,
                resample=Image.NEAREST), dtype=np.float32, copy=True) > 0.5
            confidence = np.array(Image.fromarray(confidence).transform(
                output_size, method=Image.AFFINE, data=transform_data,
                resample=Image.NEAREST), dtype=np.float32, copy=True)
        else:
            valid_float = valid
            numerator = np.asarray(Image.fromarray(depth * valid_float).transform(
                output_size, method=Image.AFFINE, data=transform_data,
                resample=Image.BILINEAR), dtype=np.float32)
            confidence_numerator = np.asarray(
                Image.fromarray(confidence * valid_float).transform(
                    output_size, method=Image.AFFINE, data=transform_data,
                    resample=Image.BILINEAR), dtype=np.float32)
            denominator = np.asarray(Image.fromarray(valid_float).transform(
                output_size, method=Image.AFFINE, data=transform_data,
                resample=Image.BILINEAR), dtype=np.float32)
            valid = denominator >= 0.5
            depth = numerator / np.maximum(denominator, 1e-6)
            confidence = confidence_numerator / np.maximum(denominator, 1e-6)

        # Keep dense teacher depths consistent with the existing object-depth
        # target under MonoDETR's geometric crop augmentation.
        if self.depth_scale == 'normal':
            depth *= crop_scale
        elif self.depth_scale == 'inverse':
            depth /= crop_scale
        elif self.depth_scale != 'none':
            raise ValueError('Unsupported depth_scale: {}'.format(self.depth_scale))
        ...
        valid &= (
            ...
            & (confidence > self.teacher_confidence_threshold)
        )
        depth = np.where(valid, depth, 0).astype(np.float32)
        confidence = np.where(valid, confidence, 0).astype(np.float32)
        return depth[None], valid[None], confidence[None]
    ...
    def __getitem__(self, item):
        ...
        if self.teacher_depth_enabled:
            teacher_depth, teacher_confidence = self.get_teacher_depth(index, img_size)
        ...
        if teacher_depth is not None:
            teacher_depth, teacher_depth_valid, teacher_depth_weight = \
                self.transform_teacher_depth(
                teacher_depth, teacher_confidence, trans_inv,
                random_flip_flag, crop_scale)
        ...
        if teacher_depth is not None:
            ...
            targets['teacher_depth_valid'] = teacher_depth_valid
            targets['teacher_depth_weight'] = teacher_depth_weight
